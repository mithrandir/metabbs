<?php
if (!$account->has_perm($board, 'moderate') && $post->user_id != $account->id) {
	access_denied();
}
if (is_post() && ($post->user_id != 0 && $account->id == $post->user_id || $account->has_perm($board, 'moderate') || $post->user_id == 0 && $post->password == md5($_POST['password']))) {
