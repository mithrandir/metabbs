		...
		<? foreach ($board->get_recent_comments(5) as $comment) { ?>
			<li><?=link_to($comment->body, $comment)?> <cite><?=$comment->name?></cite> <abbr title="<?=meta_format_date_rfc822($comment->created_at)?>"><?=meta_format_date("%m/%d", $comment->created_at)?></abbr></li>
		<? } ?>
		</ul>
