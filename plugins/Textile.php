...
class Textile_ {
   ...
   * @public
   */
  function Textile_($options = array()) {
    ...
    $this->options['char_encoding'] = (isset($this->options['char_encoding']) ? $this->options['char_encoding'] : 0);
...
/**
  ...
  function _cb($function) {
    $current =& Textile_::_current_store($this);
    return create_function('$m', '$me =& Textile_::_current(); return ' . $function . ';');
  } // function _cb
 ...
 /* static */ function &_current() {
   $current =& Textile_::_current_store($null = NULL);
   return $current[0];
 } // function _current
} // class Textile
...
class Textile extends Plugin {
	...
	function format(&$model) {
		...
		$textile = @new Textile_;
		$model->body = @$textile->process($model->body);
	}
}

register_plugin('Textile');
?>
