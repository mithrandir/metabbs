...
function process_hompage(url) { new Ajax.Updater('homepage-box', url, { method: 'get' }); }
function add_feed(url) { new Ajax.Updater('homepage-box', url, { method: 'post', parameters: { feed_url : $('feed_url').value } });	}
</script>
<form method="post" action="<?=url_for('feedengine', 'add', array('url'=>urlencode(url_with_referer_for('account', 'edit'))))?>" id="feed-form" onsubmit="add_feed(this.action);return false;">
<h2><?=i('Homepage')?></h2>
<div id="homepage-box">
