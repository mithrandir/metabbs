...
<tr>
	...
	<td>
	<? if (!$member->is_admin()) { ?>
	<a href="<?=url_for($board, 'members')?>?action=toggle&amp;id=<?=$member->id?>"><?=$member->admin ? '일반 회원으' : '관리자'?>로 전환</a> |
	<a href="<?=url_for($board, 'members')?>?action=drop&amp;id=<?=$member->id?>">관계자 목록에서 제거</a>
	<? } else { ?>
	-
	<? } ?>
	</td>
