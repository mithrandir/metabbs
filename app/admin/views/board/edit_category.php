...
<tr>
	...
	<td><a href="<?=url_admin_for($category, 'rename')?>" onclick="editCategory('category-<?=$category->id?>', this.href); return false"><?=i('Rename')?></a> |
	<?= $category->is_first() ? i('Move up') : "<a href=\"".url_admin_for($category, 'up')."\">".i('Move up')."</a>" ?> |
	<?= $category->is_last() ? i('Move down') : "<a href=\"".url_admin_for($category, 'down')."\">".i('Move down')."</a>" ?> |
	<a href="<?=url_admin_for($category, 'delete')?>" onclick="return window.confirm('<?=i('Are you sure?')?>')"><?=i('Delete')?></a>	</td>
</tr>
<? } ?>
</table>
