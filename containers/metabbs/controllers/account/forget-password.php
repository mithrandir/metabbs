...
//					$url = METABBS_HOST_URL.url_for('account','reset-password').'?id='.$user->id.'&code='.$code;
					$message = "다음 링크 '<a href=\"".full_url_for_metabbs('account','password', array('id'=>$user->id, 'code'=>$code))."\" onclick=\"window.open(this.href); return false;\">비밀번호 초기화</a>'를 클릭해서 비밀번호를 변경하시길 바랍니다.";
				...
				$error_messages->add('Your e-mail account is empty', 'user');
		}
	} else
