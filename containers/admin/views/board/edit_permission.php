...
<tr>
	...
	<td>
	<? if (!$member->is_admin()) { ?>
	<a href="<?=url_for_admin('board', 'toggle_member', array('id'=> $board->id, 'member_id' => $member->id))?>?action=&amp;id=<?=$member->id?>"><?=$member->admin ? '일반 회원으' : '관리자'?>로 전환</a> |
	<a href="<?=url_for_admin('board', 'drop_member', array('id' =>$board->id, 'member_id' => $member->id))?>">관계자 목록에서 제거</a>
	<? } else { ?>
	-
	<? } ?>
	</td>
...
<form method="post" action="<?=url_for_admin('board', 'add_member', array('id' =>$board->id))?>">
